...
import com.lxy.whv.ui.view.xlist.XListView;
import com.lxy.whv.util.LogUtils;
import com.lxy.whv.util.SimpleNetTask;
...
public class BaseListView<T> extends XListView implements XListView.IXListViewListener, AdapterView.OnItemClickListener, AdapterView.OnItemLongClickListener {
    ...
    public void
    init(DataFactory<T> dataFactory, BaseListAdapter<T> adapter) {
    ...
    public void loadWhenInit() {
        if (isLoading.get()) {
            LogUtils.d("directly return in refresh");
            return;
        }
        isLoading.set(true);
        final int skip = 0;
...
//                            Utils.toast(getContext(), R.string.chat_base_list_view_listEmptyHint);
    ...
    public void onLoadMore() {
        if (isLoading.get()) {
            LogUtils.d("directly return in loadMore");
            return;
        }
        isLoading.set(true);
